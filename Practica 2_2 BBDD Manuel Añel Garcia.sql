...
select nombre
    ...
    inner join Servicio
    on Servicio.numreg = Empleado.numreg
    where Servicio.cods = (select cods from empleado where nombre like 'Jorge Alonso Alonso');
...
select Habitacion.numero, tipo
	from Habitacion
    inner join Reserva
    on Habitacion.numero = Reserva.numero
...
select Empleado.*
    ...
    having count(distinct Limpieza.numero) = (select count(numero) from Habitacion);
...
select Cliente.*
	from Cliente
    inner join Factura
	on Cliente.dni = Factura.dni
		inner join Reserva
		on Reserva.dni = Factura.dni
			inner join Habitacion
            on Habitacion.numero = Reserva.numero
	group by cliente.dni
    having count(distinct tipo) = 2;

# 8. Obtener un listado de los proveedores cuyas facturas hayan sido aprobadas únicamente por el encargado y no por el responsable de un servicio.